...
def initial_position(k):
    ...
    pub_initial = rospy.Publisher('right_position_baxter', Vector3, queue_size=1)
    value_1= InfoClass_1()
    rospy.Subscriber('/robot/limb/right/endpoint_state', EndpointState, value_1)
    ...
    while(k>0):      
        ...
        if  position_1.pose.position.z<0.16 and \
            position_1.pose.position.z>0.04 and \
            position_1.pose.position.y>-0.5 and \
            position_1.pose.position.y<-0.2 and \
            position_1.pose.position.x<0.7 and \
            position_1.pose.position.x>0.4 :
            k=k-1
            print(k)
            if k%1000==0:
                print(k)
...
def get_point(k):
    ...
    while (k>0) :
        value_1= InfoClass_1()
        rospy.Subscriber("right_baxter_color",Vector3, value_1)
...
def go_position(k,z):
    pub_go = rospy.Publisher('robot/limb/right/joint_command', JointCommand, queue_size=1)
    ...
    value_2= InfoClass_2()
    rospy.Subscriber('/robot/limb/right/endpoint_state', EndpointState, value_2)
    pub_near = rospy.Publisher('right_position_baxter', Vector3, queue_size=1)
    ...
    while(wait>0):    
        ...
        if  position_near.pose.position.z<z-0.35 and \
            position_near.pose.position.z>z-0.50 and \
            position_near.pose.position.y>y_send-0.15 and \
            position_near.pose.position.y<y_send+0.15 and \
            position_near.pose.position.x<x_send+0.15 and \
            position_near.pose.position.x>x_send-0.15 :
            wait=wait-2
            if wait%1000==0:
                print(wait)
...
def go_box(k):
    ...
    value_3= InfoClass_3()
    rospy.Subscriber('/robot/limb/right/endpoint_state', EndpointState, value_3)
    pub_box = rospy.Publisher('right_position_baxter', Vector3, queue_size=1)
    ...
    position_better.z=position_actual.pose.position.z+0.7#Min 0.4
    print "Init up"
